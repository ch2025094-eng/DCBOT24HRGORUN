...
async def on_message_edit(self, before, after):
    ...
    before_content = before.content or "（無內容）"
    after_content = after.content or "（無內容）"
    ...
    embed.add_field(name="修改前", value=before_content[:1000], inline=False)
    embed.add_field(name="修改後", value=after_content[:1000], inline=False)
...
async def on_guild_channel_create(self, channel):
    ...
    embed.add_field(name="操作者", value=f"{user}")
